...
function swap(el1, el2) {    
    ...
    let temp2 = el1.innerHTML;    //third variable for swapping
...
function createNewArray(noOfBars = 60) {
    ...
    if(c_flag){
        ...
        // creating an barArray of user numbers
        for (let i = 0; i < size; i++) {
            barArray.push(parseInt(prompt("Enter Number : " )));
        }
    ...
    for (let i = 0; i <size; i++) {
        ...
        bar.style.height = `${barArray[i]*4}px`;
    ...
    for (let i = 0; i < noOfBars; i++) {
        barArray.push(Math.floor(Math.random() * 100));
    }
    ...
    for (let i = 0; i < noOfBars; i++) {
        const bar = document.createElement("div");
        bar.style.height = `${barArray[i]*3}px`;
        bar.innerText=barArray[i];
        bar.classList.add('bar');
        bar.classList.add('flex-item');
        bar.classList.add(`barNo${i}`);
...
const customArrayButton = document.querySelector(".custom");


customArrayButton.addEventListener("click", function(){
